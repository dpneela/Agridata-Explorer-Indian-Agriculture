...
SELECT DATABASE();
SHOW TABLES;
SELECT * FROM district_level_data;
select count(*) from district_level_data;


#1.Year-wise Trend of Rice Production Across States (Top 3)
SELECT `State Name`,
       SUM(`RICE PRODUCTION (1000 tons)`) AS Total_Rice_Production
FROM district_level_data
GROUP BY `State Name`
...
SELECT `Dist Name`,
       (AVG(CASE WHEN `Year` = (SELECT MAX(`Year`) FROM district_level_data) 
                 THEN `WHEAT YIELD (Kg per ha)` END) -
        AVG(CASE WHEN `Year` = (SELECT MAX(`Year`)-4 FROM district_level_data) 
...
SELECT `State Name`,
       ( (SUM(CASE WHEN `Year` = (SELECT MAX(`Year`) FROM district_level_data)
                   THEN `OILSEEDS PRODUCTION (1000 tons)` END) -
          SUM(CASE WHEN `Year` = (SELECT MAX(`Year`)-4 FROM district_level_data)
                   THEN `OILSEEDS PRODUCTION (1000 tons)` END)
        )
        / NULLIF(SUM(CASE WHEN `Year` = (SELECT MAX(`Year`)-4 FROM district_level_data)
...
WHERE `State Name` IN ('Uttar Pradesh', 'Punjab', 'Madhya Pradesh', 'Bihar', 'West Bengal') -- replace with your top 5
  AND `Year` >= (SELECT MAX(`Year`) - 9 FROM district_level_data)  -- last 10 years
GROUP BY `Year`, `State Name`
ORDER BY `Year`, `State Name`;